...
{
    public class AddTypeDAO:ResoucesDAO
    {
        /// <summary>
        /// 资产增长类型管理类
        /// </summary>
        ...
        {
            string str = "select * from AddType";
        ...
        {
            string str = string.Format("insert into addType values('{0}')", res.Name);
            return helper.ExecuteNonQuery(str)>0;
        }

        public override bool Del(int id)
        {
            string str = string.Format("delete from addType where id={0}", id);
            return helper.ExecuteNonQuery(str) > 0;
        }

        public override bool Modify(Resources res)
        {
            string str = string.Format("update addType set Type='{0}' where id={1}", res.Name, res.Id);
            return helper.ExecuteNonQuery(str)>0;
        }
    }
}
